...
export default function Home() {
  ...
  return (
    <div className="relative min-h-screen text-slate-900 antialiased selection:bg-blue-500/10 overflow-x-hidden font-sans">
      ...
      <div
        className={`absolute top-0 w-full transition-all duration-500 ease-in-out z-50 ${statusMessage && view === "dashboard" ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"}`}
      >
        <div
          className={`text-center py-2.5 px-4 text-xs font-medium border-b shadow-sm ${
            statusType === "success"
      ...
      {view === "landing" && (
        <div
          className={`max-w-6xl mx-auto px-6 py-20 min-h-screen flex flex-col justify-between transition-all duration-700 ease-in-out ${
        ...
        >
          ...
          <header className="flex items-center justify-between w-full">
            <div className="flex items-center gap-2.5">
              <div className="h-8 w-8 rounded-xl bg-gradient-to-br from-slate-800 to-black flex items-center justify-center text-white font-bold text-sm tracking-wider shadow-md">
                R
              </div>
          ...
          <main className="my-auto text-center space-y-8 max-w-3xl mx-auto py-12">
            ...
            <p className="text-base sm:text-lg text-slate-500 max-w-xl mx-auto leading-relaxed">
              Find qualified business contacts, clean out the noise, and deploy
              targeted sequence pipelines from a single dashboard.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-16 max-w-2xl mx-auto text-left">
              <div className="bg-white/60 backdrop-blur-md border border-slate-200/50 p-5 rounded-2xl shadow-sm">
                <div className="text-blue-600 font-semibold text-xs uppercase tracking-wider mb-1">
                  01 / Deep Extract
                </div>
                <h3 className="text-sm font-semibold text-slate-800 mb-1">
                  Contact Page Scanner
                </h3>
                <p className="text-xs text-slate-400 leading-normal">
                  Deep scans site paths to locate hidden corporate profiles.
                </p>
              </div>
              <div className="bg-white/60 backdrop-blur-md border border-slate-200/50 p-5 rounded-2xl shadow-sm">
                <div className="text-indigo-600 font-semibold text-xs uppercase tracking-wider mb-1">
                  02 / Clean Lead
                </div>
                <h3 className="text-sm font-semibold text-slate-800 mb-1">
                  Smart Heuristics
                ...
                <p className="text-xs text-slate-400 leading-normal">
                  Automatically strips media, support, and invalid system hooks.
                </p>
              </div>
              <div className="bg-white/60 backdrop-blur-md border border-slate-200/50 p-5 rounded-2xl shadow-sm">
                <div className="text-emerald-600 font-semibold text-xs uppercase tracking-wider mb-1">
                  03 / Launch Outbox
                </div>
                <h3 className="text-sm font-semibold text-slate-800 mb-1">
                  Direct Sequences
                ...
                <p className="text-xs text-slate-400 leading-normal">
                  Send transactional sales layouts instantly to target files.
                </p>
              </div>
            </div>
          </main>
          ...
          <footer className="text-center text-xs text-slate-400 pt-8 border-t border-slate-200/40">
            &copy; {new Date().getFullYear()} Reechly Inc. Built for
            lightning-fast outbound operations.
          </footer>
        </div>
      )}
      ...
      {view === "dashboard" && (
        <div
          className={`max-w-7xl mx-auto p-6 md:p-8 space-y-8 pt-16 transition-all duration-700 ease-out transform ${
            animateDashboard
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-6 border-b border-slate-200/60">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-2">
                Autonomous B2B lead generation & dynamic cold outreach platform.
              </p>
            </div>
            ...
            <div className="lg:col-span-7 bg-white/80 backdrop-blur-xl rounded-2xl border border-slate-200/60 shadow-xl shadow-slate-200/20 overflow-hidden flex flex-col transition-all duration-300 hover:shadow-2xl hover:shadow-slate-200/30">
              ...
              <div className="p-5 space-y-5">
                    ...
                    <table className="w-full text-left border-separate border-spacing-0">
                      <thead className="bg-slate-50/80 backdrop-blur-md border-b border-slate-200 text-xs font-semibold uppercase tracking-wider text-slate-500 sticky top-0 z-10">
                        <tr>
                          ...
                          <th className="px-5 py-4 border-b border-slate-200/60">
                            Found Email
                          </th>
                          <th className="px-5 py-4 border-b border-slate-200/60">
                            Source URL
                        ...
                        {leads.length === 0 ? (
                          ...
                          leads.map((lead) => (
                            ...
                            >
                              ...
                              <td className="px-5 py-3.5 font-medium text-slate-700 font-mono text-xs">
                                {lead.email}
                              </td>
                              <td
                                className="px-5 py-3.5 text-slate-400 text-xs max-w-[240px] truncate"
                                title={lead.website}
                              >
                                <a
                                  href={lead.website}
                ...
                <div className="flex items-center justify-between text-xs text-slate-400 pt-1 px-1">
                  ...
                  <span className="font-semibold text-black bg-slate-100 px-3 py-1 rounded-full">
                    {leads.filter((l) => l.selected).length} Targets Selected
                  </span>
                </div>
              </div>
            </div>
